...
import historiaSlugBg from '../../../../public/images/historiaSlugBg.jpg';
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `/na-historia/${post.slug}`,
    },
    openGraph: {
      title: post.title,
      description,
      type: 'article',
      url: `/na-historia/${post.slug}`,
      images: [
        {
...
export async function generateStaticParams() {
  ...
  const allHistoria = allPosts.filter(
    (post) =>
      post.type.map((type) => type.label).includes('Na História') &&
      post.status === 'published'
  );

  // const posts = getDocumentSlugs('posts');
  return allHistoria.map((post) => {
    return {
      historiaSlug: post.slug,
    };
  });
}
